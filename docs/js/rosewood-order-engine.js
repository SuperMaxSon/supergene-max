...
/* 아이템 아트 — 파일명이 곧 item_code (docs/img/items/<code>.png). 문서 기준 상대경로라
   docs/ 안의 두 페이지(프로토타입·오더 시뮬레이터)에서 똑같이 풀린다.
...
const ITEM_DB = [
  ...
  [211,212,7,1,0,"정밀 공구 세트","Precision Tool Set","R_TOOL",0,0,0,[]],
  [212,213,8,1,0,"전문가 공구 캐비닛","Expert Tool Cabinet","R_TOOL",0,0,0,[]],
  [213,214,9,1,0,"복원 장비 카트","Restoration Cart","R_TOOL",0,0,0,[]],
  [214,0,10,1,0,"장인 공구 컬렉션","Master Tool Collection","R_TOOL",0,0,0,[]],
  ...
  [813,814,9,1,0,"축하 케이크","Celebration Cake","R_BAKE",0,0,0,[]],
  [814,815,10,1,0,"디저트 카트","Dessert Cart","R_BAKE",0,0,0,[]],
  [815,0,11,1,0,"연회 디저트 테이블","Dessert Banquet","R_BAKE",0,0,0,[]],
  ...
  [1312,0,10,1,0,"로즈우드 리넨 컬렉션","Rosewood Linen Collection","R_LINEN",0,0,0,[]],
  ...
  [3201,3202,-1,0,0,"심플 상자","Simple Pack","X_CH32",5,0,0,[[2701,4],[2901,6]]],
  [3202,0,-1,0,0,"팬시 상자","Fancy Pack","X_CH32",11,0,0,[[2701,8],[2702,2],[2901,10],[2902,2]]],
];
...
/* [event_id, band_seq, score_base, score_min, score_max, token_pct, token_fix]
   ...
   벤치와 「오더 시뮬레이터」의 이벤트 점수가 갈렸다(Lv30 에서 94 대 24). 이제 같은 표를 본다. */
const EVENT_DB = [
...
/* ======================================================================
   ...
   ====================================================================== */
/* 엔진 판본 — 손댈 때마다 올리는 카운터를 걷었다. 증가폭에 뜻이 없었고, 정본이 1.1 인데
...
const BUILD = "정본 1.6 · 데이터 2026-09-11-export";
...
/* 기본 추첨 비중을 읽는 자리 하나 — order_item.weight.
   ...
   이 문장은 <b>v1.6(2026-09-15)에서도 한 글자도 안 바뀌었다</b> — 그래서 출처는 v1.5 로 둔다.
   바뀐 것은 <b>규칙이 걸리는 행</b>이다. v1.6 「order_item.weight / weight_multiple —
   데이터·명세 확정」이 사례를 둘 붙였다:
     「<b>604의 비중 0도 원작 대응값이며 현재 행은 비활성이다</b>」
     「<b>502는 활성이나 비중0이므로 추첨 후보에서 제외한다</b>」
   즉 이 규칙은 이제 「604 하나 때문」이 아니라 <b>502 때문에</b> 반드시 있어야 한다.
...
function generateOrder(slotNo, opts = {}) {
  ...
  /* 후보 조건 — 정본은 넷을 건다: 「order_item 에 등록 · 해금 레벨 도달 ·
     ...
     그러니 「시트로는 잴 수 없다」는 예전 서술은 <b>이제 틀렸다</b>. 연결도 예외도 시트에 있다.
     직접 생산 연결은 produce_item_N 이 답하고(생성기 52행 전부 produce_item_1 이 차 있다 · 실측),
     원작 First·SecondLevelGeneratorChain 에 해당하는 <b>예외 체인</b>은 v1.4(2026-09-14 14:23)가
     시트에 되살린 두 열이 답한다 — <b>item_spec.first_generator_chain_id ·
     second_generator_chain_id</b>. <b>0 이면 기본 연결을 그대로 쓴다</b>는 뜻이라, 예외를
     가진 체인에만 값이 든다. 시트가 끝내 답할 수 없는 것은 <b>해금 이력</b> 하나뿐이고,
     그건 세이브가 답한다.

     <b>v1.5 가 그 예외의 실값까지 박아 뒀다</b> — <b>item_spec.chain_id = 5</b>(차 체인 · item_code 501~510):
       · first_generator_chain_id = <b>6</b>(찻잔 체인) → <b>직접 생산</b> 생성기 <b>605</b>
       · second_generator_chain_id = <b>4</b>(찻주전자 체인) → <b>주문 해금 검사</b>용 생성기 <b>405~411</b>
     둘이 갈리는 이유가 여기 있다. 차를 실제로 뽑아 주는 것은 605 하나지만, 「차 주문을 열어
     줄 만큼 진도가 나갔나」는 405~411 쪽에 묻는다. 직접 생산 생성기만 보면 판정이 어긋난다.

     <b>그런데 우리 데이터(2026-09-11 export)에는 그 두 열이 아직 없다 — 실측이다.</b>
     그래서 여기는 여전히 늘 참을 돌려준다. 막힌 곳이 정본에서 <b>export 판</b>으로 옮겨갔을
     뿐이다 — 새 export 를 받아야 채울 수 있다.

     ③ 의 「보류」는 그 두 열을 채운 뒤에도 여기서 못 한다 — 분포를 보려고 켠 도구가
  ...
  /* <b>weight &gt; 0</b> — 정본 「order_item.in_use 가 참인 아이템만 사용한다.
     ...
     <b>현행 근거는 502 다</b> — 정본 v1.6(2026-09-15) 「order_item.weight /
     weight_multiple — 데이터·명세 확정」: 「<b>502는 활성이나 비중0이므로 추첨 후보에서
     제외한다</b>」 · 「<b>604의 비중 0도 원작 대응값이며 현재 행은 비활성이다</b>」.
     온라인 시트에서 604 는 비활성이 됐고, 그 자리를 활성·비중0 인 502 가 이어받았다.
     그러니 이 필터를 「604 한 행 때문에 넣은 것」으로 읽고 지우면 안 된다.

     <b>다만 우리 입력이 낡았다</b> — 2026-09-11 export 에는 <b>502 가 아예 없고
     604 가 활성</b>이다(실측). 그래서 이 줄이 <b>지금</b> 걸러 내는 행은 여전히
     604 꽃무늬 찻잔(체인 6 · step 4 · unlock_level 7) 하나뿐이고, 그 자리 범위에
     들 때 종수가 1 만큼 줄어든다. <b>새 export 를 받으면 걸러지는 대상이
     604 → 502 로 바뀐다</b> — 규칙은 그대로고 걸리는 행만 갈린다.
  ...
  for (let seat = 1; seat <= slotMax; seat++) {
    ...
    const rows = cand.map((o) => {
      ...
      /* 가중식 = <b>기본 비중 × 상황 배수 ÷ 반복 제수</b>. 정본 원문:
         ...
         기본 비중은 order_item.weight 다. <b>우리 입력(2026-09-11 export)</b>에서는
         전 행 100 이고 604 한 행만 0 이라, 604 는 안 나오는 게 맞다 — 그 0 이
         「원작 대응값」이라는 것이 정본 v1.6 문장이다(「604의 비중 0도 원작 대응값이며
         <b>현재 행은 비활성이다</b>」). 온라인 시트에서 현행 비중0 활성 행은 <b>502</b> 이고,
         어느 쪽이든 base 0 이라 뽑히지 않는다 — 후보 종수에서 빼는 일은 [3] pool 이 한다.

...
function generateOrder(slotNo, opts = {}) {
  ...
  /* <b>weight &gt; 0</b> — 정본 「order_item.in_use 가 참인 아이템만 사용한다.
     ...
     <b>2026-09-15 export 로 입력이 실물과 맞았다</b> — 502 가 활성·비중 0 으로 들어왔고
     604 는 비활성으로 내려갔다(실측). 그래서 이 줄이 지금 걸러 내는 행은 <b>502</b> 다.
     한동안은 604 하나였는데, 규칙은 그대로고 <b>걸리는 행만 갈린 것</b>이다.
  ...
  for (let seat = 1; seat <= slotMax; seat++) {
    ...
    const rows = cand.map((o) => {
      ...
      /* 가중식 = <b>기본 비중 × 상황 배수 ÷ 반복 제수</b>. 정본 원문:
         ...
         기본 비중은 order_item.weight 다. <b>2026-09-15 export</b> 기준 활성 74행 중
         비중 0 인 행은 <b>502</b> 하나이고, 정본 v1.6 이 「502는 활성이나 비중0이므로
         추첨 후보에서 제외한다」고 못 박았다. 604 도 비중 0 이지만 <b>지금은 비활성</b>이다
         (「604의 비중 0도 원작 대응값이며 현재 행은 비활성이다」).

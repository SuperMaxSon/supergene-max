...
/* ────────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────── */
/* 판매 확인창 — 정본의 열 이름은 <b>`rare`</b> 다. `show_sell_confirm` 은 시트에도 웹에도
   없고 우리 변환기가 붙인 별칭이다(scripts/xlsx2balance.py:285 · `rare` 를 그대로 복사).
   개명은 예고만 됐고 오지 않았다 — 신판 export 36탭 전수 grep 0건이고, 밸런스시트 웹도
   「`rare` : 1이면 판매 전 확인창을 띄운다」로 쓴다. <b>시트에서 이 이름을 찾지 마라.</b>
   값은 신판에서 실제로 찼다 — 234행 중 rare=1 이 94행이다(구판은 236행 전부 0 이었다).
   따라서 아래 판매가 임계 폴백은 <b>지금 데이터에서는 안 탄다</b>. 칸이 통째로 빈 구판을
   불러왔을 때만 살아나는 안전망으로 남겨 둔다 — sheetHasConfirm() 이 그 판정이다. */

  ...
  },
  {
    ...
    cards: [
      ...
      },
      {
        ...
        desc: "인게임 구조를 표가 아니라 도식 7장으로 세운 문서입니다. 실선은 코드가 있는 칸, 점선은 정본 기획에는 있고 코드가 0줄인 칸이라 어디가 비었는지가 색과 선만 보고 갈립니다. 2026-09-16 15:40 재실측으로 갱신했습니다 — 인게임이 5,229줄에서 9,220줄로 커지면서 오더 추첨 8단계(OrderDraw 876줄)와 레일 뷰가 들어와 12단계 중 11번이 닫혔고, 지금은 마지막 «마감»(실패 UX·연출)에 서 있습니다. 러너는 15종 816 PASS / 0 FAIL. 아웃게임은 같은 기간 0줄 그대로입니다. 상태를 PlayerVM(계정)과 IngameVM(세션)으로 가른 기준, 판정과 적용을 2메서드로 나눈 이유, 저장 키가 3개로 갈린 경위까지 한 바퀴로 읽힙니다.",
        url: "docs/story-merge-ingame-architecture.html",
        project: "smg",
        status: "완료",
        version: "v1.3",
        updated: "2026-09-16 16:23",
        pinned: false,
        tags: ["코드 실측", "구조"],
        share: false,
      },
  ...
  },
  {
    ...
    cards: [
      ...
      },
      {
        ...
        desc: "AOS에서 토너먼트가 붙은 컨텍스트의 판 클리어 지면을 공유(A · 빌드 496)에서 생성(B · 빌드 497)으로 바꾸면 수락률이 오르는지 보는 A/B입니다. 두 빌드가 갈리는 건 AOS · 토너 있는 컨텍스트 · 판 클리어 세 조건이 모두 맞을 때뿐이라 분모를 그 조건으로 좁혔고, 어느 칸이 갈리는지를 2단계 판별 도식으로 세웠습니다. 판정 지표는 지표마다 A · B · 차이(pp) · 상대증감(%) 네 칸으로 세워 두고, 주 지표 2개와 가드레일 3개에 채택 조건까지 정의했습니다. 측정 후 숫자만 채우면 되는 틀입니다. 09-15~오늘 누적(09-18 08:49 기준) 실측을 담았습니다 — 지면 수락률 A 8.3%(363/4383) · B 20.4%(269/1321), FB 점수 등록률 6.4% → 15.8%. 09-17 부터 486 이 빠져 두 빌드가 전량 서빙이고 군 배분도 거의 반반이라, 문서의 채택 조건으로 판정할 수 있는 구간입니다. 문서 하단에는 이번 빌드부터 들어간 배너 광고 KPI를 붙였습니다 — 전달·품질은 분모 대비 비중이 보이도록 막대로 세웠고(시도 87.5% · 로드 성공 66.5% · 실패 33.4%), 실패 21,686건의 구성은 미지원 클라이언트 66.2% · 재고 없음 16.0% · 기타 17.9% 스택 막대입니다. 노출 지속 중위 53초, 노출군 vs 미노출군 부작용 대조와 BigQuery 쿼리 2개가 함께 있습니다.",

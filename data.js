...
const SECTIONS = [
  ...
  },
  {
    ...
    cards: [
      {
        title: "QA 이슈 트래커 — 어떻게 끝났나",
        desc: "채널에 이슈가 쌓이는 동안 계속 갱신하는 상시 문서(현재 23건) — 갱신 절차를 문서 안에 고정해 뒀다. 이슈 한 줄로 훑고 궁금한 것만 눌러 펼치는 목록. 펼치면 증상 → 원인 → 지금 코드의 파일:라인 → 커밋이 나오고, 커밋을 누르면 커밋 히스토리의 해당 항목으로 바로 넘어간다(왜·어떻게는 그쪽, 여기는 결말만). 16건 수정 완료 · 4건 논이슈 · 2건 확인 대기 · 실제 미종결은 1건뿐. 완료 표시가 없다고 미해결이 아니라는 점(종결 사유는 스레드 마지막 댓글에만 있다), lastDailyDirectEntryAt 읽기 경로 하나가 3건으로 갈라져 접수된 사례, 원인 중 가장 큰 덩어리(6건)가 버그가 아니라 벤치 4종과의 구현 차이였다는 점이 핵심.",

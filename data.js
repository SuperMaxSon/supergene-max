...
const SECTIONS = [
  {
    ...
    cards: [
      ...
      },
      {
        ...
        desc: "세 단이 모두 다이어그램인 드릴다운 레퍼런스. 1단 오픈맵으로 전 지면(인트로 → 로비 → 프리게임 → 인게임 → 결과 → 사후 플로우)을 훑고, 지면을 누르면 탭 안에 그 지면의 가지 다이어그램(지면 루트 → 버튼 가지 → 도착 지면), 가지를 누르면 호출 사슬 플로우 다이어그램(단계 상자 → 연결선 → 도착)이 열린다. 사슬의 지면 이동 상자를 누르면 그 지면의 2단 다이어그램으로 계속 타고 간다. 지면 노드 42 · 흐름 57 · 2단 가지 230(최장 7단계) · onClick 핸들러 182개 실측.",

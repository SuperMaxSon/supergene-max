  ...
  },
  {
    ...
    cards: [
      ...
      },
      {
        ...
        desc: "AOS에서 토너먼트가 붙은 컨텍스트의 판 클리어 지면을 공유(A · 빌드 496)에서 생성(B · 빌드 497)으로 바꾸면 수락률이 오르는지 보는 A/B입니다. 두 빌드가 갈리는 건 AOS · 토너 있는 컨텍스트 · 판 클리어 세 조건이 모두 맞을 때뿐이라 분모를 그 조건으로 좁혔고, 어느 칸이 갈리는지를 2단계 판별 도식으로 세웠습니다. 판정 지표는 지표마다 A · B · 차이(pp) · 상대증감(%) 네 칸으로 세워 두고, 주 지표 2개와 가드레일 3개에 채택 조건까지 정의했습니다. 측정 후 숫자만 채우면 되는 틀입니다. 09-15~16 QA 표본(09-16 16:23, 오늘 19시까지 매시 갱신)을 채우고 있습니다 — 지면 수락률 A 5.3%(36/684) · B 22.1%(23/104), FB 점수 등록률 5.5% → 11.2%. 다만 QA 표본이라 판정용은 아닙니다(라이브 대세는 아직 486). 문서 하단에는 이번 빌드부터 들어간 배너 광고 KPI를 붙였습니다 — 전달·품질은 분모 대비 비중이 보이도록 막대로 세웠고(시도 81.6% · 로드 성공 67.2% · 실패 32.6%), 실패 78건의 구성은 미지원 클라이언트 63.3% · 재고 없음 16.8% · 기타 19.9% 스택 막대입니다. 노출 지속 중위 51초, 노출군 vs 미노출군 부작용 대조와 BigQuery 쿼리 2개가 함께 있습니다.",

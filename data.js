...
const SECTIONS = [
  {
    ...
    cards: [
      ...
      {
        title: "UI 제작 TODO",
        desc: "어느 버튼이 어느 팝업을 여는지를 인게임/아웃게임 두 축으로 펼친 드래그·줌 맵. 기본은 T0 만 보이고 티어를 켤수록 층이 붙습니다. 만들 것은 뷰 15 · 팝업 49 = 64개, 33개는 기준 화면 재사용이라 만들지 않습니다.",
      ...
      {
        title: "저장 데이터 TODO",
        desc: "기획이 요구하는 영속 저장 항목 137개를 인게임 53 · 아웃게임 84 로 갈라 필드 단위로 펼치고 구현 여부만 표시한 판. 구현 37 · 미구현 93 · 서버 대기 7.",
  ...
  },
  {
    ...
    cards: [
      ...
      {
        title: "스토리머지 인게임·아웃게임 아키텍처",
        desc: "두 축의 구조를 고정폭 도식 5장으로 세운 문서입니다. 직전 판본(v1.4 · 09-18)의 «아웃게임 5층이 통째로 비었다»는 전제가 깨졌습니다 — 그 축이 4,122줄로 서서(마을 · 채팅 러너 · 꾸미기 · 심부름) 이제 인게임과 대등한 두 축입니다. 그래서 이 판본은 «어디가 비었나»가 아니라 «어디서 무엇을 주입하고 어디에 저장하나»를 그립니다: setClock → init → bootstrap → refillRail 의 순서 계약, 저장 키 5개가 갈린 이유, 왕복이 onHideStart 를 안 타서 생기는 재진입 함정 세 가지. 인게임은 14,467 → 17,162줄, ViewOutgame 이 ViewDecorate 670줄을 흡수했고 시트에 main_task 197행이 들어왔습니다. 대신 검증망이 무너졌습니다 — npm test 2종은 개명된 프리팹(ViewStory.prefab)을 가리켜 죽어 있고, Node 러너 16종은 BalanceProvider 가 game/data 로 이사한 뒤 하네스 복사 목록이 낡아 14종이 모듈을 못 찾습니다. 복사 목록에 한 줄을 더해 돌려 보면 11종 672 PASS / 18 FAIL 이고, 그 18건의 귀속(러너가 낡은 건지 제품이 틀린 건지)은 이 문서에서 가르지 않았습니다.",
      ...
      {
        title: "심부름 보상 시트",
        desc: "런칭분 197건이 무엇을 받고 무엇을 무는지를 그림째 폈습니다. 코인·경험치·추가 아이템을 전부 실제 게임 아트로 세워, 코드(2501)만 보고는 알 수 없던 「무엇이 나오는가」가 바로 보입니다. 행을 누르면 그 심부름에서 오가는 대화가 채팅 화면으로 펼쳐집니다. 이번에 재생 시점 축을 바로잡았습니다 — 본편 2,762줄 중 2,568줄(93.0%)이 task_start, 즉 그 심부름을 「하기 전」에 나옵니다. 완료 시점은 94줄, 구역 완료는 100줄뿐입니다. story.story 는 scene_id 와 항상 같지 않아서(구역 완료는 area_01 → area_01_finish 로 접미사가 붙습니다) 정확 일치로만 찾으면 그 대사가 통째로 빠집니다. 그래서 표를 심부름 단위가 아니라 이벤트 단위로 다시 세웠습니다 — 대사 행과 심부름 행이 실제 플레이 순서대로 번갈아 놓이는 414행 타임라인입니다. 왼쪽에 끊기지 않는 세로 줄기를 긋고 행마다 점을 찍었습니다(채운 큰 점 = 심부름, 빈 작은 점 = 대사). day 마다 밴드를 넣어 구역과 그날의 심부름·대사 분량을 먼저 보여 줍니다 — 구역은 day 구간과 1:1이라 행마다 반복하지 않습니다. 한 심부름은 「시작 전 대사 → 심부름 → (완료 대사) → (구역 완료 대사)」로 풀립니다. 씬 번호는 재생 순서가 아닙니다(day 1 만 봐도 01→60→02→03·04→50→53→51→61→52 순이고, 번호는 작성 시점입니다) — 번호순으로 읽으면 이야기가 깨집니다. 데이터는 2026-09-21 15:39 export 기준으로 전량 다시 뽑았고, 그 과정에서 시트 안의 시차가 드러났습니다 — main_task·story 는 197건 신판인데 string_code 는 188개라 신규 9건에 영문 제목이 없고, skin_deco 54키 중 택1을 가리키는 건 44개뿐이라 택1 61자리 중 17자리에 A·B·C 가 없습니다. 코인·경험치·대사는 시트에서, 제목·구역·비포애프터·디자인은 화면 기획서에서 옵니다 — 두 소스는 197건·유형이 전건 맞물리지만 코인 값은 162건 어긋나며 이 표는 시트를 씁니다. 데코 반영 현황은 꾸미기 자리 139개 중 클라에 들어온 건 day 1 의 5개뿐, 필요한 데코 아트 400장(수선 156 + 택1 244)은 0장입니다.",
        url: "docs/chore-reward-sheet.html?v=20260922h",
        project: "smg",
        status: "완료",
        // ⚠ version 은 이 표가 읽어온 **소스 판본**이다 — 2026-09-21 15:39 export(57탭)를
        //    직접 읽고, 밸런스 시트 기획서 원문 판본(v1.11 · 56탭)을 적었다.
        //    문서를 고쳤다고 올리는 개정 번호가 아니다. 시트를 다시 받을 때만 움직인다.
        version: "v1.11",
        updated: "2026-09-22 12:14",
        pinned: false,
        tags: ["시트 실측", "머지", "스토리"],
        share: false,
      },
  ...
  },
  {
    ...
    label: "도구",
    accent: "tool",
    // 열어서 값을 만지는 페이지만 여기 둔다. 읽는 기록물(커밋 히스토리 · 이식 가이드)은
    // "작업 기록" 으로 간다 — 한 섹션에 섞으면 분석 탭이 겪던 문제가 그대로 재발한다.
    // 판별 기준은 tags 의 "인터랙티브" — 열어서 조작하고 값을 가져가는 페이지.
    ...
    cards: [
      ...
      {
        title: "오더 시뮬레이터",
        desc: "밸런스시트 JSON 을 넣으면 벤치와 같은 엔진으로 8단계를 돌려서, 어긋난 항목이 엔진·시트·판정 중 어디 탓인지까지 답합니다. 타입별 난이도 폭과 슬롯 1~5 를 한 줄로 뽑는 리얼 레일까지 봅니다. 2026-09-15 시트 기준.",
      ...
      {
        title: "고정 오더",
        desc: "게임 시작부터 순번대로 나가는 order_fixed 16행. 추첨이 아니라 시트에 박힌 값이라 오더 시뮬레이터가 skipFixed 로 빼고 도는 경로입니다. normal 해금이 Lv3 이라 Lv1~2 오더는 전량 여기 있습니다.",
        url: "docs/rosewood-order-fixed.html",
        project: "smg",
        status: "초안",
        version: "정본 1.6",
        updated: "2026-09-15 18:20",
      ...
      {
        title: "프로토타입",
        desc: "Rosewood 코어루프를 직접 눌러 보는 인게임 시뮬레이터. 2026-09-15 시트 export 실물 239종 35체인이 그대로 돌아갑니다. 아웃게임은 후순위라 접어 뒀습니다.",
      ...
      {
        title: "초기 보드",
        desc: "initial_board 63칸을 7×9 그대로 깔고 재생을 누르면 클라 판정으로 머지를 두고, 둘 게 없으면 생성기를 눌러 산출을 빈 칸에 떨어뜨린 뒤 다시 머지를 봅니다. 잠금 없는 칸 2개로 시작해 어디까지 풀리는지, 무엇이 먼저 마르는지(재고·에너지·빈 칸) 보는 판입니다.",
  ...
  },
  {
    ...
    cards: [
      {
        title: "CP 원격설정 3380 트래킹",
        desc: "CP 팝업을 시트 원격 설정 + 프리팹 통합으로 바꾼 빌드 3380의 롤아웃 KPI 트래킹입니다. 같은 날 함께 서빙되는 3375를 대조로 두고 CP 퍼널(노출·CTR·닫기)·PLAY 전환(FAIL·재시작)·회귀 감시(판수/세션·5분 세션·광고·클라 에러) 12개 지표를 세워 뒀고, 지표마다 판정에 필요한 최소 분모를 박아 표본이 모자란 행은 잠가 뒀습니다 — 표본이 차면 그 행부터 열립니다. 이번 배포의 미결 항목은 PLAY 후 20초 내 재시작률입니다: FB 테스트 빌드(9/10)에서 9건 중 2건이 대상 게임 대신 우리 게임 재시작으로 끝나 코드 원인을 전부 배제하고 보류했던 건이고, 정식 빌드에서 구버전 수준(2.7%)이면 종결·10% 초과면 회귀로 봅니다. 3380의 내부 QA 로그(9/10·9/11·9/15, 9/10은 CP 강제 노출)는 계산에서 제외했습니다. 9/16 14:06 기준 점유 2.0%(DAU 703)로 램프 중이라 아직 판정 문서가 아닙니다.",
      ...
      },
      {
        ...
        desc: "AOS에서 토너먼트가 붙은 컨텍스트의 판 클리어 지면을 공유(A · 빌드 496)에서 생성(B · 빌드 497)으로 바꾸면 수락률이 오르는지 보는 A/B입니다. 두 빌드가 갈리는 건 AOS · 토너 있는 컨텍스트 · 판 클리어 세 조건이 모두 맞을 때뿐이라 분모를 그 조건으로 좁혔고, 어느 칸이 갈리는지를 2단계 판별 도식으로 세웠습니다. 판정 지표는 지표마다 A · B · 차이(pp) · 상대증감(%) 네 칸으로 세워 두고, 주 지표 2개와 가드레일 3개에 채택 조건까지 정의했습니다. 측정 후 숫자만 채우면 되는 틀입니다. 09-15~오늘 누적(09-21 08:46 기준) 실측을 담았습니다 — 지면 수락률 A 8.1%(931/11465) · B 20.4%(661/3247). 09-17 부터 486 이 빠져 두 빌드가 전량 서빙이고 군 배분도 거의 반반이라, 문서의 채택 조건으로 판정할 수 있는 구간입니다. 문서 하단에는 이번 빌드부터 들어간 배너 광고 KPI를 붙였습니다 — 전달·품질은 분모 대비 비중이 보이도록 막대로 세웠고(시도 88.7% · 로드 성공 65.2% · 실패 34.6%), 실패 59,687건의 구성은 미지원 클라이언트 65.4% · 재고 없음 16.3% · 기타 18.3% 스택 막대입니다. 노출 지속 중위 52초, 노출군 vs 미노출군 부작용 대조와 BigQuery 쿼리 2개가 함께 있습니다.",
  ...
  },
  {
    ...
    cards: [
      ...
      {
        title: "솔리테어 커밋 히스토리",
        desc: "커밋 문구로 훑고 필요한 것만 펼치는 목록. 07-13~09-14 Max 커밋 181건 diff 전량 실측.",

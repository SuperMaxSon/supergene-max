...
const SECTIONS = [
  {
    ...
    cards: [
      {
        title: "공유 실패 — 원인과 고칠 것",
        desc: "피드 공유 성공률 43.8%(코지·퍼블마 79%)의 원인을 결론부터 쓴 문서. 어쩔 수 없는 게 아니라 우리 로직 문제 3건 — ① 로그 tournament_size 하나 채우려고 판 종료마다 getPlayersAsync를 1초마다 30회 재시도한다(벤치 3종은 이 FB 호출이 0건, 마종은 5 하드코딩). 남은 in-flight 호출이 공유 다이얼로그를 PENDING_REQUEST로 거절시킨다. ② 지연 실행이 아무 클릭이나 트리거로 써서 결과창 공유 버튼을 누르면 피드 공유와 토너 공유가 100ms 간격으로 겹친다. ③ FB 다이얼로그 직렬화가 없다(벤치도 없지만 벤치는 던지는 호출 수가 적다). 근거는 실패의 81%가 PENDING_REQUEST이고 시도당 발생률이 퍼블마의 3.6배(26.5% vs 7.3%), 유저 취소는 3건. 파라미터 가설 2개는 실측으로 사망 — iOS·웹 intent 이탈 구간(Windows)에서 우리는 93.4%로 벤치와 같고, 격차는 전부 Android(27.2% vs 퍼블마 64.8%). 어쩔 수 없는 몫은 Android 편중 75.6%로 약 16pt. 퍼널 전체로는 우위가 전환 성공 3.647(1위) 하나뿐이고 공유·메시지·생성은 전부 최하위. 프로젝트 대조표는 부록으로 접었다.",
        url: "docs/viral-surface-cross-project.html",
        status: "진행중",
        version: "v5.0",
        updated: "2026-08-18 17:51",
        pinned: false,
        tags: ["바이럴", "공유", "PENDING_REQUEST", "원인분석", "5프로젝트", "OS분해", "BigQuery", "가설반증"],
      },
      {

    ...
    cards: [
      {
        ...
        desc: "3374 ↔ 3375. 두 빌드는 이 처치 말고 코드가 같습니다. 생성 시도는 −38%인데 실제 생성 성공은 +5.8%로 늘었고 다른 지면 이탈도 없습니다. 잔존은 D+N 11구간 중 8곳에서 B가 높지만 전 구간 유의성 미달(최소 p=0.127)이라 무승부이며, 처치 층만 떼면 9:2(p≈0.065)까지 갑니다.",
        url: "docs/coin-match-tournament-reject-ab.html",
        project: "cmt",
        status: "진행중",
        version: "v1.9",
        updated: "2026-09-07 12:20",
        pinned: false,
        tags: ["토너먼트", "BigQuery", "A/B"],
      },
      {
        title: "토너 클리어 소셜 슬롯 A/B",
        desc: "A(485 교대) ↔ B(486 공유 전용) · 9/3~9/6 나흘치. 주지표(공유 성공/DAU)가 +65%로 유의하게 올랐고(p<.001), 초대를 전부 없앴는데도 유입 총량은 오히려 +4%입니다. 통과율은 7.7% → 5.6%로 떨어졌지만 시도를 2.3배 밀어 성공 건수를 늘렸습니다. 리텐션·생성률·배정 균형은 이상 없습니다.",

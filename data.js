    ...
    cards: [
      {
        ...
        desc: "AOS에서 토너먼트가 붙은 컨텍스트의 판 클리어 지면을 공유(A · 빌드 496)에서 생성(B · 빌드 497)으로 바꾸면 수락률이 오르는지 보는 A/B입니다. 두 빌드가 갈리는 건 AOS · 토너 있는 컨텍스트 · 판 클리어 세 조건이 모두 맞을 때뿐이라 분모를 그 조건으로 좁혔고, 어느 칸이 갈리는지를 2단계 판별 도식으로 세웠습니다. 판정 지표는 지표마다 A · B · 차이(pp) · 상대증감(%) 네 칸으로 세워 두고, 주 지표 2개와 가드레일 3개에 채택 조건까지 정의했습니다. 측정 후 숫자만 채우면 되는 틀입니다. 09-15~16 QA 표본을 1차로 채웠습니다 — 지면 수락률은 A 16.7%(2/12) · B 25.0%(2/8)이고 FB 점수 등록률은 21.7% → 37.5%로 올랐지만, 군당 20여 명 표본이라 판정용은 아닙니다(라이브 대세는 아직 486). 문서 하단에는 이번 빌드부터 들어간 배너 광고 KPI를 붙였습니다 — 로드 성공률 66.2%, 실패의 85.1%가 CLIENT_UNSUPPORTED_OPERATION(미지원 클라이언트), 노출 지속 중위 41초, 그리고 노출군 vs 미노출군 부작용 대조와 BigQuery 쿼리 2개입니다.",
        url: "docs/sol-clear-create-slot.html",
        project: "sol",
        status: "측정 중",
        version: "v1.3",
        updated: "2026-09-16 11:13",
        pinned: false,

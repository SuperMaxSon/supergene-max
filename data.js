...
const SECTIONS = [
  {
    ...
    cards: [
      ...
      },
      {
        ...
        desc: "주 대비 주(US, 전주 8/3~9 vs 당주 8/10~16) + 마종 대조 + 창 종류×OS 분해. 총 바이럴 12.8%→32.2%지만 상승분의 101%가 초대 신설이고 기존 4채널은 제자리(같은 주 마종은 무변화). 판수 101%·광고 107%로 활동성·수익화는 이미 마종 초과. 구멍 셋의 성격이 이번에 완전히 갈렸다. ① 메세지(격차 67%) — 성공률 100%인데 판당 발사율이 마종의 1/7~1/15. 순전히 안 쏘는 문제이고, 마종·코지도 매칭 서버 없이 클라 스테이지 타입으로 처리하므로 로비 진입점 하나 추가로 해결 가능(파일 2~4개·서버 불필요). ② 토너 생성(44%) — 데스크톱에서는 생성 6.69% ≈ 공유 6.81%로 창 종류가 구별되지 않는데 모바일에서만 0.26% vs 10.05%로 38.7배 갈린다. 좁은 가로 화면에서 특정 창만 죽는다는 직접 증거. ③ 토너 공유(17%) — 판당 발사율은 솔 26.57% vs 마종 21.82%로 우리가 1.22배 더 쏜다(게이트 4개를 다 거치고도). 격차 100%가 수락률(8.38% vs 30.86%)이고, 창은 뜬다(실패의 84.4%가 USER_INPUT). 기술 에러를 다 제외해도 3.7배→3.3배로 11%만 설명. 데스크톱 2.3배 × 모바일 추가 1.7배로 분해된다. 중요 — 여러 결론의 근거였던 '초대 95%'를 철회한다. 4개 OS에서 92~99% 균일이고 3470이 inviteAsync().then()에 붙어 있어 유저 선택이 아니라 프라미스 resolve를 찍는다(유입률 1.14%). 우리 바이럴 61%가 이 지표 위에 서 있어 재정의 전까지 초대 확장 권고는 보류. iOS는 별건 — 마종은 iOS=Android(34.2 vs 36.6)인데 솔만 iOS가 1/10, 생성은 273시도 0건. 원인 후보 17개 배제.",
        url: "docs/viral-orientation-gap.html",
        status: "진행중",
        version: "v7.0",
        updated: "2026-08-18 16:35",
        pinned: false,

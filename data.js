    ...
    cards: [
      {
        ...
        desc:
          "클리어 후 공유가 거절되던 두 원인의 수정 결과. 리더보드에 점수가 없어서 거절(INVALID_OPERATION)은 공유 시도 대비 8.3% → 0.4%로 사실상 사라졌고, 광고와 공유창이 서로를 막는 거절(PENDING_REQUEST)은 5.1% → 4.0%로 부분 개선. 후자는 락을 기다리는 대신 결과창에서 창을 광고보다 먼저 띄우도록 순서를 고정한 것이라, 결과창 밖에서 겹치는 구간은 남아 있다. 수정 전 빌드 456, 최신 458 기준 — 458에서 기록 필드 662건과 점수 등록 호출 4.4배 증가로 반영이 확인됐다. 남은 4.0%를 락 보유자별로 분해해보니 토너먼트 창이 광고를 막던 건은 435→15건(-97%)으로 끝났고, 남은 것은 유저당 23건이 5명에게 몰린 광고 스턱 세션(클라 회수 불가, FB 문의)과 공유창 68건(회수 가능하나 0.8%p라 보류)이다.",

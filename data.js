...
const SECTIONS = [
  {
    ...
    cards: [
      ...
      {
        title: "전체 지면 플로우 맵 — 드래그·확대로 보는 오픈맵",
        desc: "솔리테어 전 지면(인트로 → 로비 → 프리게임 → 인게임 → 결과 → 사후 플로우)을 한 장에 올리고, 지면마다 어떤 버튼이 어떤 메소드를 부르는지 코드에서 뽑아 붙인 레퍼런스. 화면에 다 안 들어오므로 드래그로 옮기고 휠로 확대해서 보고, 카드를 누르면 그 지면의 버튼 → 메소드 전체와 앞뒤 흐름이 열린다. 지면 노드 42 · 흐름 57 · onClick 핸들러 182개 실측.",

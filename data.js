...
const SECTIONS = [
  {
    ...
    cards: [
      ...
      },
      {
        ...
        desc: "주 대비 주(전주 8/3~9 vs 당주 8/10~16, US) + 마종 대조. 총 바이럴이 12.8%→32.2%로 올랐지만 상승분의 101%가 초대 신설이고 기존 4채널은 0.500→0.491로 제자리 — 같은 주 마종은 무변화라 시장 요인도 아니다. 광고는 +66%로 마종을 넘었고(7.16 vs 6.71) 판수도 101%라 활동성·수익화는 문제가 아니다. 남은 구멍 셋 — 메세지(격차 67%) · 토너 생성(44%) · 토너 공유(17%). ① 메세지는 성공률 100%인데 판당 발사율이 마종의 1/7~1/15라 순전히 '안 쏘는' 문제이고, 마종도 매칭 서버 없이 클라 추천+무탭 전환으로 하므로 로비에 진입점 하나만 추가하면 된다(파일 2~4개·서버 불필요). ② 생성은 모바일만 미작동(Android 0.26%·iOS 0건)인데 같은 Android에서 초대 창은 94.96%가 먹혀 'FB 창이 안 먹힌다'는 폐기 — createAsync 하나로 좁혀졌고 가로(사내 7개 중 우리만)가 1순위. 원인 후보 12개 배제.",

  ...
  },
  {
    ...
    cards: [
      ...
      },
      {
        ...
        desc: "AOS에서 토너먼트가 붙은 컨텍스트의 판 클리어 지면을 공유(A · 빌드 496)에서 생성(B · 빌드 497)으로 바꾸면 수락률이 오르는지 보는 A/B입니다. 두 빌드가 갈리는 건 AOS · 토너 있는 컨텍스트 · 판 클리어 세 조건이 모두 맞을 때뿐이라 분모를 그 조건으로 좁혔고, 어느 칸이 갈리는지를 2단계 판별 도식으로 세웠습니다. 판정 지표는 지표마다 A · B · 차이(pp) · 상대증감(%) 네 칸으로 세워 두고, 주 지표 2개와 가드레일 3개에 채택 조건까지 정의했습니다. 측정 후 숫자만 채우면 되는 틀입니다. 09-15~오늘 누적(09-18 09:04 기준) 실측을 담았습니다 — 지면 수락률 A 8.5%(376/4426) · B 20.4%(270/1326). 09-17 부터 486 이 빠져 두 빌드가 전량 서빙이고 군 배분도 거의 반반이라, 문서의 채택 조건으로 판정할 수 있는 구간입니다. 문서 하단에는 이번 빌드부터 들어간 배너 광고 KPI를 붙였습니다 — 전달·품질은 분모 대비 비중이 보이도록 막대로 세웠고(시도 87.6% · 로드 성공 66.5% · 실패 33.4%), 실패 21,869건의 구성은 미지원 클라이언트 66.2% · 재고 없음 16.0% · 기타 17.8% 스택 막대입니다. 노출 지속 중위 52초, 노출군 vs 미노출군 부작용 대조와 BigQuery 쿼리 2개가 함께 있습니다.",

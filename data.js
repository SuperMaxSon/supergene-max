...
const SECTIONS = [
  {
    ...
    cards: [
      {
        title: "주간 KPI 3축 — 전주 대비 8/10~8/16",
        desc: "지난주가 겨냥한 3개 축(광고 노출·플레이 횟수·컨텍스트와 공유)을 전주 8/3~8/9 vs 당주 8/10~8/16, US 기준 유저-일 단위로 나란히 측정. ① 축1 광고는 달성 — 유저당 노출 0.93 → 3.83회(4.12배)이고 도달률 8.0 → 32.8%(4.10배)로 배율이 겹쳐, 증가분 전부가 도달에서 나왔다(1인 강도 11.6 → 11.7 불변). 원인은 프리로드 재고 확인 제거이며 지면 쿨타임은 30초 그대로. 대가는 노출 실패 — 성공률 94.9 → 68.6%, 주 13,139건, switch_cancel은 절반 실패. ② 축2 플레이는 혼재 — 판수 평균 −5.9%인데 p50은 2 → 3으로 하단만 올랐다(직행 발동률 50~54%가 후보). 재도전 2223은 미조회. ③ 축3 공유는 총량이 아니라 낭비가 줄었다 — 유저당 성공은 1.38 → 1.39로 제자리인데 실패가 8,844 → 5,630(−36%), 합계 성공률 28.0 → 38.4%. P2P초대 신설 94.7%, 토너포스트 49.9 → 96.0%. 다만 토너공유는 7.5 → 8.4%로 방치돼 당주 공유 실패의 97%가 이 채널 하나이고, 전환 팝업 닫기는 +107%로 과노출 신호. 토너 생성은 성공률 1.3 → 2.6%지만 절대 성공은 167 → 97(−42%)이고 전주 수치는 실패(13,670) > 시도(12,412)로 신뢰 불가.",
        url: "docs/weekly-kpi-0810-0817.html",
        status: "완료",
        version: "v3.0",
        updated: "2026-08-18",
        pinned: false,
        tags: ["주간KPI", "3축", "전주대비", "전면광고", "공유초대", "컨텍스트전환", "US기준", "p50"],
      },
      {
        title: "NRU 0~10분 — 6개 프로젝트 온보딩 비교",
        desc: "지표 → 우열 → 가설 → 원인 4단계. ① 같은 정의로 재고 있는 지표는 9개 중 4개뿐. ② 그 4개는 3승 1패 — 10분 도달률 16.6%·세션당 판수 3.66·체류 270초 1위, 유일한 열세는 첫 판 완료율 46.5 vs 마종 54.3(−7.8pt). ③ 신규 — 값이 왜 갈리는지 반증 가능한 가설 7건을 편차 4버킷(계측·코호트·구조·온보딩로직)으로 분해. 우리 우위를 깎는 것부터 검증한다: 5-A 더퍼즐 최하위 1.6%는 타이머 원점 차이일 수 있다(6개 리포 실측 — 5개는 인트로 addLink('login','loginLog') 동일 노드, 더퍼즐만 GameManager.ts:652 playStageOnStart 뒤 + onDestroy clear), 5-B 우위 3개는 서로 독립 확증이 아니라 판 길이의 세 얼굴일 수 있다(코지가 부분 반증), 5-C 10분 도달자 판수는 코지 16.1로 1위라 코호트 차이 신호. 열세 쪽은 3-B 신설 — 이탈이 분량(13탭)이 아니라 딜 연출 1.3~1.8초 입력 무시일 수 있어 stage_list를 먼저 건드리면 잘못된 레버. 그 외 1-B 번들 전송 바이트, 4-A 튜토 잣대 N=3 제안. ④ 측정 계획에 코드로 오늘 되는 Tier-0 2건과 N판 도달률 추가. 중복 서술·철회된 튜토리얼 인과표는 문서에서 내림.",

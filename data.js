...
const SECTIONS = [
  {
    ...
    cards: [
      ...
      {
        title: "솔 바이럴이 낮은 이유 — 확인·분석·개선",
        desc: "확인 → 분석 → 개선 순서. ① 토너 생성률이 모바일에서만 무너진다 — Windows 6.03% / Android 0.17% / iOS 0.00%(마종은 반대로 모바일이 데스크톱의 2.1배). 개편 세 구간 내내 미동 없음, 개편 전에도 Android 약 7,400시도 0건이라 상시 결함. ② 같은 화면에서 공유율은 세 OS 모두 우상향(Windows +107%, 목표 대비 73%)이라 유저 태도·피로로 설명 불가 — 남는 차이는 다이얼로그 크기. 사내 7개 중 우리만 가로(1280×720)이고 모바일 가로는 세로 공간 ~360px. 실패의 73%가 USER_INPUT = 열리고 나서 닫힌다. ③ 실기 녹화 30분이면 판정 — 확인 버튼이 화면 안에 있는가. 가설 15개 중 11개 기각.",
        url: "docs/viral-orientation-gap.html",
        status: "진행중",
        version: "v2.0",
        updated: "2026-08-18 15:14",
        pinned: false,

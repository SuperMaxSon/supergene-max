...
const SECTIONS = [
  {
    ...
    cards: [
      ...
      },
      {
        ...
        desc: "주 대비 주(US, 전주 8/3~9 vs 당주 8/10~16) + 마종 대조 + 지역×OS 분해. 총 바이럴 12.8%→32.2%지만 상승분의 101%가 초대 신설이고 기존 4채널은 제자리(같은 주 마종은 무변화). 판수 101%·광고 107%로 활동성·수익화는 이미 마종 초과. 구멍 셋 — 메세지(격차 67%) · 토너 생성(44%) · 토너 공유(17%). 토너 생성은 서로 다른 두 문제로 갈렸다 — ① 모바일 전면 붕괴는 전 지역 공통(US Android 0.17% · 비US 0.25% · iOS 양쪽 0.00%)인데 같은 모바일의 공유율은 12.9~14.3%로 데스크톱보다 높아 지역·기기가 아니라 창 종류 문제이고, ② 데스크톱은 비US 15.65%로 마종(15.82%)과 같은데 US만 6.03%다(신규 항목, 원인 미규명). 전세계 생성률이 높아 보이는 건 성공의 89%가 데스크톱인 믹스 착시. esther 41번과의 수치 차이는 정의가 아니라 지역·기간 — 재현 검증으로 cr_ok/sh_ok/msg_ok/유저-일 전부 일치 확인. 메세지는 성공률 100%인데 발사율이 마종의 1/7~1/15라 순전히 안 쏘는 문제이고, 마종도 매칭 서버 없이 하므로 로비 진입점 하나 추가로 해결 가능(파일 2~4개·서버 불필요). 원인 후보 14개 배제.",
        url: "docs/viral-orientation-gap.html",
        status: "진행중",
        version: "v6.0",
        updated: "2026-08-18 16:02",
        pinned: false,
